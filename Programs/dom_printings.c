...
int m=LEN+1,n=LEN+2;
int  mxColor;
int tempmat[LEN+1][LEN+2],matrix[LEN+1][LEN+2];
int  c2[LEN+1], deg[LEN+1];  
 void colorvertex( int );

int main(){
    int i,j,k,co[m],bu[m][m],ran[m]; //degree list,temporary degree matrix,color identification matrix,matrix to store covered vertices,randomly removed nodes list
    ran[0]=1;
	...
	{ if(co[i]==0)
	  ...
	  if(c==1)
	   ...
	   //for(k=1;k<m-1;k++) if(deg[k]==deg[k+1])count++;
	   //if(count==m-1) i=m;	
	...
	printf("\n\n");
	for(j=1;j<m;j++){for(k=1;k<m;k++){
			printf("%d\t",tempmat[j][k]);if(tempmat[j][k]==1)deg[j]++;
	   }printf("\n");}
	   printf("\n\n");
	for(k=1;k<m;k++){printf("%d\t ",deg[k]);}

...
//#define FILENAME "p_hat300-2.clq"
int max(int *,int,int);
...
int main(){
	...
	int big,loc,dum;//sorting according to max degree degree first
   
   	for(i=0;i<LEN;i++){ 
             big=fixed[list[i]][n-1];
             loc=i;
	     for(j=i+1;j<LEN;j++){
			if (fixed[list[j]][n-1]>big){big=fixed[list[j]][n-1];
                                                     loc=j;}
                                 } 
...
int max(int *list,int depth,int len){
	...
	independent_list=calc_ind(list,len,&ilen);
		...
		adjacent_list=calc_adjacent_list(independent_list[i],list,len,&alen);
	     //if(alen=0){ if (depth>max){ Max=depth; return;}}
         if(depth+alen>Max)
                       {

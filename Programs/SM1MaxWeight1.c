...
int no=0;
int lit=0;
int select_Max_Var(int ,int,int,int,int);
int Greedy_Swap(int);
...
int main(){
  ...
  for(i=1;i<LEN;i++)
   { if(Max>=target) break;
       t=i; lit=0; increment=0; int restart=0.8*target;//1+rand()%LEN; //printf("\nt=%d\n",t);
      for(j=1;j<m;j++)
      ...
      for(k=0;k<restart;k++)
	   ...
	   //else if(increment>target/2) break;  //pruning
        if(Max>=target) break;
  ...
  while(cflen>0)
     { it++; lit++;//iteration++
       if((clen+cflen)<=Max)return;
        i=select_Max_Var(cflen,c0len,clen,it,x);
       if(c0[i]<=0) break; 
       c[++clen]=c0[i]; 
   if((it<0.25*target)&&(cc0[1][i]<0.5*target)) {cc0[1][i]=cc0[1][i]+x; } //target/5 for MANN_45 
...
//...........................................................................
int select_Max_Var(int cflen,int c0len, int clen, int it,int x)//done//change
    { int i,j,k,c0pos=0,c0max=0,flag=0,list[2][cflen], cc0avg=999;  
         
       for(k=1;k<m;k++){if((c0[k]>0)&&(cc0avg>cc0[1][k])) cc0avg=cc0[1][k]; }
		for(k=0;k<m;k++){if(c0[k]>0){flag=1;c0pos=k;c0max=cc0[0][k];break;}}
      if(flag==1)
         {for(j=k+1;j<m;j++)
           {if(c0[j]>0)
		     {//if((cc0[0][j]>c0max)&&(cc0[1][j]<it)) 	  
			 // if((cc0[0][j]>c0max)&&((cc0[1][j]<it)||((it<3)&&(cc0[0][j]<lit))))
             if((cc0[0][j]>c0max)&&((cc0[1][j]<it)||((it<0.25*target)&&(cc0[1][j]<=(cc0avg+x)))))
